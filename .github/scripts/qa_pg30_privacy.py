#!/usr/bin/env python3
"""ORIGEX PG30 Privacy QA — legal-demo, noindex, TOC, responsive and accessibility gate."""
...
SECTION_IDS = [
    "information-collected", "use", "cookies", "third-parties",
    "retention", "rights", "contact", "update-date"
]


def source_qa():
    report = {"failures": [], "pages": {}, "css": {}}
    ...
    for marker in (".orx-legal-shell", ".orx-legal-toc", "position: sticky", "max-width: 991.98px", "max-width: 767.98px"):
        if marker not in css:
            css_failures.append("css-missing:" + marker)
    report["css"] = {"failures": css_failures}
    report["failures"] += css_failures

    specs = {
        "ar": {
            "dir": "rtl",
            "h1": "سياسة الخصوصية — نموذج توضيحي قابل للتخصيص.",
            "canonical_intro": "هذا نص نموذجي يوضح هيكل صفحة سياسة الخصوصية داخل القالب. يجب مراجعته وتعديله بواسطة صاحب الموقع أو مستشاره القانوني وفقًا لطريقة جمع البيانات والقوانين المطبقة قبل النشر.",
        ...
        "en": {
            ...
            "canonical_intro": "Sample privacy structure only; buyer/legal adviser must adapt to actual data practices and applicable law.",
    ...
    for lang, spec in specs.items():
        ...
        if len(h1s) != 1 or h1s[0].get_text(" ", strip=True) != spec["h1"]:
            failures.append("h1")
        if spec["canonical_intro"] not in text:
        ...
        if not toc:
            ...
            hrefs = [a.get("href") for a in toc.select("a[href^='#']")]
            expected = ["#" + item for item in SECTION_IDS]
            if hrefs != expected:
        ...
        for section_id in SECTION_IDS:
            ...
            if not heading or not heading.get("id"):
                failures.append("section-heading:" + section_id)
        ...
        if re.search(r"\b20\d{2}[-/]\d{1,2}[-/]\d{1,2}\b", text):
            failures.append("fabricated-policy-date")

        contact = soup.select_one('[data-orx-email="trade"]')
        if not contact or not contact.get("href", "").startswith("mailto:"):
            failures.append("contact-config-hook")
        ...
        for banned in ("gdpr", "ccpa", "qatar pdpl", "saudi pdpl", "uae pdpl"):
            if banned in lowered:
                failures.append("named-law-claim:" + banned)
        for marker in ("cookie-banner", "consent-banner", "data-cookie-consent", "localstorage", "sessionstorage"):
            if marker in lowered:
                failures.append("fake-consent-tech:" + marker)
        ...
        if soup.find("script", src="../assets/js/origex-privacy.js"):
            failures.append("unnecessary-page-runtime")
        if "TARGET" in text or "targetft" in lowered:
        ...
        report["pages"][lang] = {
            "failures": failures,
            "sections": len([s for s in SECTION_IDS if soup.find("section", id=s)]),
            "tocLinks": len(toc.select("a[href^='#']")) if toc else 0,
            "iconCount": len(icons),
        }
            ...
            try:
                for width in (390, 820, 1366, 1536):
                    ...
                    try:
                        ...
                        if not toc.is_displayed():
                            failures.append("toc-hidden")
                        ...
                        if driver.find_element(By.CSS_SELECTOR, '[data-pg30-updated]').text.strip() not in ("Replace before production", "يُستبدل قبل النشر"):
                            failures.append("updated-placeholder")
                        ...
                        if width >= 1051:
                            ...
                            driver.find_element(By.CSS_SELECTOR, "[data-orx-drawer-close]").click(); time.sleep(.05)
                ...
                try:
                    ...
                    time.sleep(.08)
                    if driver.execute_script("return location.hash") != "#information-collected":
                        failures.append("toc-hash")
                    target = driver.find_element(By.ID, "information-collected")
                    if target.rect["y"] <= 0:
                        failures.append("toc-target")
                    switch = driver.find_element(By.CSS_SELECTOR, ".orx-lang-switch")
                    expected = "../en/privacy.html" if lang == "ar" else "../ar/privacy.html"
                    if expected not in switch.get_attribute("href"):
                        failures.append("language-switch")
                except Exception as exc:
                    failures.append(type(exc).__name__ + ":" + str(exc)[:240])
                report["interaction"][lang + "-toc-language"] = failures
                report["failures"] += [lang + "-toc-language:" + failure for failure in failures]
            finally:
                driver.quit()
    finally:
        server.terminate()
